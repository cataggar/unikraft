...
The native producer writes the fixed JIT preset only when creating a fresh `.config`;
...
# Use --mode aot, fast or full only with the matching prepared identity.
```

The native producer tests cover all variant identities, exact generated bytes,
private file modes and native failure/refusal boundaries. The app-owned native
optional parser is exercised with synthetic snapshot/sampler unit, fault,
property and retained-Python differential fixtures (including the pinned SDK
sampler when available). The installed native CLI ports the source-pinned sampler and has no runtime
SDK path or Python fallback. Its JSON-v1 object has only schema, version,
mode, **raw** serial byte count and SHA-256 (no `compute` for workloads);
default output is a correctness-only line. It returns 0 on validation,
1 with empty stdout and one bounded path-free refusal line, or 2 for usage.
Sampler identities must contain `jit_mode`: the `sample-aot` value is
explicit JSON `null`, while JIT requires the matching `"fast"` or `"full"`
string. Missing is not equivalent to `null`; snapshot identities may omit
this sampler-only key.
The Python command remains an uncalled differential reference while current
production callers await their separate cutover. Neither path admits boot or
measurement evidence.
For each variant use a fresh output root: `prepare` refuses existing artifacts,
source export or consumer. `verify` checks the schema-1 identity and hashes;
`olddefconfig` and `native-images` retain private diagnostics on failure and
never treat a partial output as a success identity. These commands do not
establish native workload or cloud acceptance.
...
The native CLI uses the shared bounded, no-follow immutable file snapshots;
the retained Python checker uses its existing native CI read.
