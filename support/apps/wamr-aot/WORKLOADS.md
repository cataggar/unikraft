...
optional parser is exercised with synthetic snapshot/sampler unit, fault and
property fixtures plus CLI checks. The installed native CLI ports the
source-pinned sampler and has no runtime SDK path or Python fallback.
Its JSON-v1 object has only schema, version,
