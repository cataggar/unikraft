...
test "private atomic state is durable owner-only bounded and hash-bound" {
    ...
    const absolute = try fixture.directory.dir.realPathFileAlloc(io, "state.json", allocator);
    defer allocator.free(absolute);
    var expected: ?contracts.Sha256 = digest;
    ...
    defer optional_unbound.deinit();
    var literal_unbound = try files.readSensitiveAbsolute(io, allocator, absolute, 4096, null);
