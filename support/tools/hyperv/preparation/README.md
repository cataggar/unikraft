...
## Fresh ORIGINAL seed production (offline #88 substage)
...
`PRIVATE_PARENT` must already be an absolute, current-user-owned mode-0700
directory. All ancestors must satisfy the shared native ownership, no-symlink
and non-writable-by-others policy. `FRESH_BASENAME` is one non-hidden component,
not a path or request file. Set `umask 077`. The command creates an exclusive
mode-0700 child; **any preexisting child is refused**, including an empty
directory, file or dangling symlink. It never overwrites, adopts or resumes.
The parent's stable `.writer.lock` serializes producers; the child has its own
stable lock. Locks are retained as files after descriptor release.

This is the supported `generate-fresh` path. Zig's OS-backed `Io.randomSecure`
generates two 128-bit identities. Nonnil lowercase hex32 identities and their
distinctness are checked; entropy errors, zeros and collisions fail rather than
falling back. No ID, RNG, external footer, disk-size, controller, SKU, synthetic
or fixture override is accepted by the production command.

The private child contains:

| File | Contract |
| --- | --- |
| `original.raw` | Exactly 4,294,967,296 bytes: 8,388,608 sectors of 512 bytes. |
| `original.vhd` | Identical complete raw prefix plus a validated 512-byte native miz fixed-disk footer; the generated disk ID is its UUID. |
| `original.json` | Legacy strict version-2 ORIGINAL manifest, consumable by the existing direct manifest reader. No new authority or producer fields are added to it. |
| `original.config` | Original guarded seed fragment with the same fresh IDs, policy 2, LUN 7, `MAX_DEVICES=2`, `MAX_LUNS=8`. **Unsolved**, not a compiled/booted image configuration. |
| `production.started.json` | Durable local start marker, explicitly not acceptance. Retained on success as well as interruption. |
| `production.json` | Separate canonical local producer self-report, binding identities, fixed geometry, and each raw/VHD/manifest/original-config path, size, mode and full SHA-256. Includes native implementation/compiler/miz/footer labels, not authenticated source provenance. |
| `.writer.lock` | Shared native stable single-link mode-0600 writer lock. |

All files are current-user-owned mode 0600 and single-link. The seed represents
one SCSI disk at LUN 7. Only the two identical policy-2 seed sectors at LBAs 8
and 9 are populated. Every other raw byte is zero, including intent 16,
receipt 17, and workload extent 32/count 16. Sparse creation is descriptor-based
and bounded-memory; it never allocates a 4-GiB buffer or reuses historical
payloads. The footer is generated by the pinned miz `Footer.forFixedDisk` API,
using the generated disk identity and the existing deterministic `ukn1` footer
contract, not an opaque footer input.

The producer scans **all** raw and VHD bytes, checks pristine regions, seed
records, exact prefix equality and the complete footer, and computes full
hashes. Controls are parsed/validated and separately hashed. It syncs each file
and directory, publishes `production.json` create-only, then reopens and fully
validates the final bindings and directory/lock custody before reporting success.
There are two complete disk-validation passes in a successful production call,
not sampling or just checking sparse extents. Memory is bounded independently
of disk size.

Errors preserve the fresh child for private diagnosis; they never delete the
parent or silently retry. Missing/invalid final records, partial publications,
short/truncated/drifting files, changed modes/owners/links, lock substitution,
and file/directory sync failures cannot return success. A visible record after
a sync or final-validation failure is **not** proof of successful publication.
Do not adopt an interrupted child or delete it to retry with those identities.
Select a different fresh basename for a later attempt. The in-process read-only
`original_seed.inspect` requires an expected record digest and revalidates the
complete local bytes; that digest still does not authenticate the producer.
...
image/config and target approvals. Historical grants, seeds and ledgers are
never read or changed.

No derived direct-image fragment is emitted. Direct REPORT LUNS uses
`MAX_LUNS=2` retained identities, not a maximum numeric LUN; silently rewriting
the original fragment would lose its byte binding. A future separately labelled
and hashed derived fragment must pass `config.validateDirectPersistence`.
Purpose-specific image solving/building, actual ELF/IRQ/image proofs, exact-image
local boot, export/import, trusted admission and cloud lifecycle/recovery remain
separate work. The six-input direct CLI and parked cloud entry points are
unchanged.

### Native seed fixtures and explicit large qualification

`test-original-seed` uses the existing Zig test runner and manageable
49/257/4096-sector streaming fixtures. It covers fresh entropy/identity
validation, direct version-2 manifest/footer compatibility, original versus
direct config refusal, complete-byte/hash comparisons, corruption/truncation,
unsafe/preexisting outputs, links/locks, short/no-progress writes and native
publication/sync failure seams. Small streaming fixtures produce no ORIGINAL
production records. This target is also included in `test`; existing synthetic,
Debug and ReleaseSafe tests and physical image gates are unchanged.

Full-size I/O is deliberately **not** added to each Debug fixture. Use the
explicit existing-build target separately on each actual host architecture:
...
The qualification root must not exist and its parent must already be private.
This noninstalled native driver executes the actual production CLI with a
20-minute bound, checks refusal of a repeat into the same child, and independently
reads/hashes all 8,589,935,104 raw/VHD bytes using the existing direct pristine,
manifest and footer readers without fabricating a live scope/approval. The
producer itself has already read both complete files twice. Budget approximately
24 GiB of logical reads across all three passes, plus hashing CPU and small
control/metadata I/O; sparse physical allocation and actual runtime depend on
the filesystem/host. No physical image assertions or existing timeouts are
relaxed.

Private `qualification.json` retains host architecture, optimization, executable
and product hashes, exact byte/count results, and measured durations.
`creation.stdout.json` retains the explicitly non-admitted CLI result. These
are **offline qualification artifacts**, not independently authenticated
provenance or seeds admitted for later execution. Runtime on both actual
AArch64 and x86_64 hosts, not cross-compilation, is required before publication.
